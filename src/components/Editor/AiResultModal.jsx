...
/**
 * Reveal modal — opens only when a background AI job has finished.
 * Shows the customer's REAL original vs their REAL AI result (drag slider),
 * then lets them keep it (consumes the free transform) or try another style.
 */
export default function AiResultModal({ originalUrl, resultUrl, label, onUse, onTryAnother, onClose }) {
  const { t } = useLang();
  ...
  return (
    ...
    >
      ...
      >
        {/* ── Real before/after slider (customer's original vs AI result) ── */}
        <div
          ref={containerRef}
          style={{
            position: 'relative',
            width: isMobile ? '100%' : '64%',
            height: isMobile ? '46vh' : 'min(82vh, 760px)',
            flexShrink: 0,
            display: 'flex',
            overflow: 'hidden',
            touchAction: 'none',
        ...
        <div className="flex flex-col flex-1 min-h-0 p-6 gap-4 overflow-y-auto">
          ...
          <div className="flex flex-col gap-2 mt-auto">
            ...
            >
              <RefreshCw size={15} /> {t('ai.tryAnotherBtn')}
            </button>
          </div>
        </div>
      </div>
